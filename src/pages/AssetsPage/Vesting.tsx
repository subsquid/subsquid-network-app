      ...
      select: res => {
        if (res?.some(r => r.status === 'success')) {
          return {
            ...
            deposited: unwrapMulticallResult(res[2])?.toString(),
            releasable: unwrapMulticallResult(res[3])?.toString(),
            released: unwrapMulticallResult(res[4])?.toString(),
            balance: unwrapMulticallResult(res[5])?.toString(),
            initialRelease: Number(unwrapMulticallResult(res[6]) || 0) / 100,
            expectedTotal: unwrapMulticallResult(res[7])?.toString(),
          };
        }
